<template>
  <div v-if="!cp_loading">
    <a @click="open = !open" id="tomodify" style="right: 0; padding-right: 2em; position: absolute">
        <button v-if="!open" class="button is-link is-light">Add custom response</button>
    ...
    <div v-if="open" class="box">
      <form @submit.prevent="addResponse" id="cpform">
        ...
        <input v-if="!sended" class="button"
      ...
      <article class="media">
        <div class="media-content">
          <div class="content">
            ...
            <a v-if="resp.link" :href="resp.link"><b><small>File link</small></b>
              </a>
            <br />
            <p>{{ resp.response }}</p>
          </div>
          <nav class="level is-mobile">
            <div class="level-left">
              <a class="level-item" aria-label="delete">
                <a @click="deleteResponse(id)" v-if="!sended" class="button is-danger is-light">Delete</a>
              </a>
            </div>
          </nav>
        </div>
...
export default {
    ...
    methods: {
        getResponses() {
            this.axios.get(`/api/guilds/${this.guildID}/cp`).then(e => {
        ...
        addResponse() {
          ...
          this.sended = true;
          this.axios.post(`/api/guilds/${this.guildID}/cp`, this.form, {
            headers: {
              "X-CSRF-Token": this.csrfToken
            }
          }).then(() => {
            this.responses.push({ match: this.form.match, response: this.form.response, link: this.form.link });
            this.open = false;
            this.sended = false;
            this.form.match = "";
            this.form.response = "";
        ...
        deleteResponse(id) {
          this.sended = true;
          this.axios.delete(`/api/guilds/${this.guildID}/cp`, {
            headers: {
              "X-CSRF-Token": this.csrfToken
            },
            data: { id }
